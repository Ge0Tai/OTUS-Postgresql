...
6. После команды <b>commit</b> во втором окне, выполняется транзакция в третьем сеансе:

![](pics/dz8/43_lock_row_3sess.PNG)

7. Ну и, наконец, после фиксации изменений в последнем окне - все блокировки уходят:

![](pics/dz8/44_lock_row_3sess.PNG)

8. Ну и заглянем в лог:

![](pics/dz8/45_log_200mc.PNG)

Видим, что в лог (по превышению 200 мс) попали 2 процесса - <b>1749</b> и <b>1777</b>.
Причём лог даёт достаточно много полезной информации:
<b>LOG:</b> <i>процесс 1749 всё ещё ждёт ShareLock на транзакцию 1610380</i>  
<b>DETAIL:</b> <i>сообщает нам, что процесс 1736 блокирует нас (1749) и мы ждём</i>  
